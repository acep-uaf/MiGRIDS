...
# 2 - subtract base from value -> increase from base case
    ...
    # check if multiple plotRes is not already a list or tuple, then make into one.

    # plotting style values
    ...
    for pR in plotRes:
        ...
        if baseSet != '' and baseRun != '':
            ...
            try:
                ...
                # get the base value
                yBase = pd.to_numeric(getPlotRes(pR0, dfBase).loc[baseRun])
            ...
            if any(dfAttr[otherCol] != dfAttr[plotAttr]) and otherCol.lower() != 'started' and otherCol.lower() != 'finished':  # make sure not identical (this is an option when setting values of 2 different attributes
                ...
                if not(removeSingleOtherAttr and len(uniqueValues) == 1):
                    if otherCol in otherAttrNames.keys():
                        # the legend entry is the y value legend, plus the other attribute legend. For example,
                        # 'Firm loads, GBS Rated Power (kW): '
                        legendNames.append(pR0Leg  + otherAttrNames[otherCol])
                    else:
                        legendNames.append(pR0Leg  + otherCol)  # add col name to legend names
